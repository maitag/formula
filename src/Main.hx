...
class Main 
{
	static function main() 
	{
		...
		var h:TermNode;
		h=TermNode.fromString("a*(b+c+d)+a*c");
//		t=TermNode.fromString("((((x+(x^2))+2)+4)+(x^5))+(x^ln(2))");
		h.simplify();
		trace(h.toString());
		h=TermNode.fromString("3*x^2+x*(-5*x+4)");
		h.simplify();
		trace(h.toString());
		//h=TermNode.fromString("x^m+x^n");
		//h=TermNode.fromString("k*(a+b)+d*(-b-a)");
		//h=TermNode.fromString("a*x+b*(0-x)");
		//h=TermNode.fromString("x^2-3*x*(4*x+2)");
		h=TermNode.fromString("(a+b)*(c-d)");
		h.expandall();
		trace(h.toString());
		h.simplify();
		trace(h.toString());
